...
import { getIntegrationHub } from "@/lib/domain-hubs";

export const metadata: Metadata = {
  title: "Integración Power Platform + Dynamics 365",
  description: "Hub de integración: Dataverse como base común, Model-Driven sobre D365, ALM, APIs/Azure y CE + F&O (dual-write, virtual tables).",
};
...
export default function IntegrationHubPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-8 animate-fade-in">
      <section className="rounded-xl border border-border bg-card px-6 py-6 shadow-fluent-1">
        <div className="flex items-start gap-4">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg" style={{ background: hub.accent }}>
            <Workflow className="h-5 w-5 text-white" aria-hidden />
          </div>
          <div className="min-w-0 flex-1 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="border-0 text-white" style={{ background: hub.accent }}>Capa puente</Badge>
      ...
      <section aria-labelledby="int-flow-heading">
        <div className="mb-4">
          <h2 id="int-flow-heading" className="text-base font-semibold text-foreground">
      ...
      <section aria-labelledby="int-sections-heading">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 id="int-sections-heading" className="text-base font-semibold text-foreground">
              Contenido por capa de integración
            </h2>
            <p className="text-sm text-muted-foreground">Módulos y labs existentes agrupados por punto de integración.</p>
          </div>
              ...
              <div className="mt-4 space-y-2">
                {section.links.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="group block rounded-lg border border-border bg-muted/20 p-3 transition-colors hover:border-[#8661C5]/40 hover:bg-[#F3EFFB] dark:hover:bg-[rgba(134,97,197,0.14)]"
                  >
                    <p className="text-sm font-medium text-foreground group-hover:text-[#8661C5] dark:group-hover:text-[#B9A3E3]">{link.title}</p>
