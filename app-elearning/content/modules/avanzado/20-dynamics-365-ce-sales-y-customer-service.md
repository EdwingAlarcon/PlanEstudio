...
- **Pause/Resume de SLA:** el temporizador de un KPI de SLA se puede pausar explícitamente (por ejemplo, cuando el caso queda "Esperando respuesta del cliente") para que el tiempo de espera del cliente no cuente en contra del agente, y se reanuda automáticamente cuando el cliente responde. Sin esta configuración, un caso que espera 3 días una respuesta del cliente aparece como una violación de SLA que en realidad no es responsabilidad del agente.
- **Métricas operativas de servicio:** un dashboard de Customer Service maduro reporta como mínimo: *First Response Time* (tiempo hasta la primera respuesta), *Resolution Time* (tiempo hasta el cierre), *SLA Success Rate* (% de casos resueltos dentro del SLA), *Backlog* (casos abiertos sin resolver, especialmente los vencidos), y *CSAT* conceptual (satisfacción del cliente, típicamente vía encuesta post-cierre). Sin estas métricas, un supervisor no puede distinguir entre un equipo con carga alta pero eficiente y uno con problemas reales de desempeño.
...
- **Autoservicio con Power Pages:** un portal de Power Pages puede exponer a los clientes la creación y consulta de sus propios casos, y la búsqueda de Knowledge Articles publicados, sin consumir una licencia de agente. Reduce volumen de casos simples (reseteo de contraseña, preguntas frecuentes ya documentadas en la KB) antes de que lleguen a una cola — el mismo patrón que ya viste con Power Pages en el Módulo 21, aplicado específicamente a autoservicio de soporte.
